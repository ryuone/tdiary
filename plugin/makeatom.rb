...
#   @conf['makeatom.hidecontent'] : hide full-text content. default: false
#   @conf['makeatom.shortdesc'] : shorter description. default: false
#   @conf['makeatom.comment_link'] : insert tsukkomi's link. default: false
#
# options to be edited in tdiary.conf:
#   @conf['makeatom.file']  : local file name of RSS file. default: 'index.rdf'.
...
#   @conf['makeatom.partial'] : how much portion of body to be in description
#                            used when makeatom.shortdesc, defalut: 0.25
	...
	module ::TDiary
		class RDFSection
			...
			def time_string
				...
				@time.strftime( "%a, %d %b %Y %H:%M:%S " ) + zone
			end
...
class MakeatomFull
	...
	def file
		f = @conf['makeatom.file'] || 'index.rdf'
	...
	def url
		u = @conf['makeatom.url'] || "#{@conf.base_url}#{File.basename(file)}"
		u = "#{@conf.base_url}#{File.basename(file)}" if u.empty?
		u
	end
end

@makeatom_rsses << MakeatomFull::new( @conf )
...
class MakeatomNoComments < MakeatomFull
	...
	def file
		f = @conf['makeatom.no_comments.file'] || 'no_comments.rdf'
		f = 'no_comments.rdf' if f.empty?
		f
	end

	def write( encoder )
		return unless @conf['makeatom.no_comments']
		super( encoder )
	end

	def url
		return nil unless @conf['makeatom.no_comments']
		u = @conf['makeatom.no_comments.url'] || "#{@conf.base_url}#{File.basename(file)}"
		u = "#{@conf.base_url}#{File.basename(file)}" if u.empty?
		u
	end
end

@makeatom_rsses << MakeatomNoComments::new( @conf )
...
def makeatom_update
	...
	require 'pstore'
	cache = {}
	rsses = @makeatom_rsses

	begin
		PStore::new( "#{@cache_path}/makeatom.cache" ).transaction do |db|
			begin
				...
				rsses.each{|rss| rss.head( makeatom_header( uri ) ) }
				cache.values.sort{|a,b| b.time <=> a.time}.each_with_index do |rdfsec, idx|
					unless rdfsec.section.respond_to?( :visible? ) and !rdfsec.section.visible?
						rsses.each {|rss|
							rss.item( makeatom_seq( uri, rdfsec ), makeatom_body( uri, rdfsec ), rdfsec )
						}
					end
					if idx > 50
		...
		File.unlink( "#{@cache_path}/makeatom.cache" )
		retry
	end
	...
	rsses.each {|r|
		r.banner( makeatom_banner( uri, rdf_image ) ) if rdf_image
		r.foot( makeatom_footer )
		r.write( Proc::new{|s| replace_entities( to_utf8( s ) )} )
	}
end

def makeatom_header( uri )
	rdf_url = @conf['makeatom.url'] || "#{@conf.base_url}index.rdf"
...
def makeatom_desc_shorten( text )
	if @conf['makeatom.shortdesc'] then
		@conf['makeatom.partial'] = 0.25 unless @conf['makeatom.partial']
		len = ( text.size.to_f * @conf['makeatom.partial'] ).ceil.to_i
		len = 500 if len > 500
	else
		len = 500
	end
...
def makeatom_body( uri, rdfsec )
	...
	if rdfsec.section.respond_to?( :body_to_html ) then
		...
		rdf << %Q|<pubDate>#{h rdfsec.time_string}</pubDate>\n|
		...
		rdf << %Q|<author>#{h @conf.author_name}</author>\n|
		unless rdfsec.section.categories.empty?
			rdfsec.section.categories.each do |category|
				rdf << %Q|<category>#{h category}</category>\n|
			end
		end
		desc = remove_tag( body ).strip
		desc.gsub!( /&.*?;/, '' )
		rdf << %Q|<description>#{h makeatom_desc_shorten( desc )}</description>\n|
		unless @conf['makeatom.hidecontent']
			text = ''
			...
			unless text.empty?
				...
				unless @conf['makeatom.comment_link']
		...
		rdf << %Q|<link>#{h uri}#{anchor rdfsec.id}</link>\n|
		rdf << %Q|<pubDate>#{h rdfsec.time_string}</pubDate>\n|
		rdf << %Q|<title>#{makeatom_tsukkomi_label( rdfsec.id )} (#{h rdfsec.section.name})</title>\n|
		rdf << %Q|<author>#{h rdfsec.section.name}</author>\n|
		text = rdfsec.section.body
		rdf << %Q|<description>#{h makeatom_desc_shorten( text )}</description>\n|
		unless @conf['makeatom.hidecontent']
...
def psh_hub
	@conf['psh.hub'] || 'http://pubsubhubbub.appspot.com/'
end

def psh_publish
	uri = URI.join( psh_hub, 'publish' )
	data = 'hub.mode=publish&'+@makeatom_rsses.select{|i| i.url}.map{|i| 'hub.url='+u( i.url )}*'&'
	header = {'User-Agent' => 'tDiary '+TDIARY_VERSION, 'Content-Type' => 'application/x-www-form-urlencoded'}
...
add_update_proc do
	makeatom_update unless @cgi.params['makeatom_update'][0] == 'false'
	psh_publish unless @cgi.params['makeatom_update'][0] == 'false'
end

add_header_proc do
	html = ''
	@makeatom_rsses.each do |rss|
		next unless rss.url
...
add_edit_proc do
	checked = @cgi.params['makeatom_update'][0] == 'false' ? ' checked' : ''
	r = <<-HTML
	<div class="makeatom"><label for="makeatom_update">
	<input type="checkbox" id="makeatom_update" name="makeatom_update" value="false"#{checked} tabindex="390">
	#{@makeatom_edit_label}
	</label></div>
	HTML
end
